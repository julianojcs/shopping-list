...
class ListTableViewController: UITableViewController {
    ...
    func showItemsFrom(_ snapshot: QuerySnapshot) {
        ...
        shoppingList = snapshot.documents.map { doc in
            return ShoppingItem(
...
//        }
    ...
    func showAlertForItem(_ item: ShoppingItem? = nil) {
        ...
        let okAction = UIAlertAction(title: "Ok", style: .default) { _ in
            guard let name = alert.textFields?.first?.text, let quantityString = alert.textFields?.last?.text, let quantity = Int(quantityString) else {return}
            let data: [String: Any] = [
                "name": name,
                "quantity": quantity
            ]
    ...
    */
    ...
    override func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCell.EditingStyle, forRowAt indexPath: IndexPath) {
        if editingStyle == .delete {
            let shoppingItem = shoppingList[indexPath.row]
            firestore.collection(collection).document(shoppingItem.id).delete(completion: { error in
                if let _ = error {
